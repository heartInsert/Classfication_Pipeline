...
class FMixBase:
    ...
    def fmix_loss(self, input, y1, index, lam, train=True, reformulate=False):
        ...
        if train and not reformulate:
            y2 = y1[index]
            return self.loss_fc(input, y1) * lam + self.loss_fc(input, y2) * (1 - lam)
        else:
            return self.loss_fc(input, y1)
...
class FMix(FMixBase):
    ...
    def __init__(self, decay_power=3, alpha=1, size=(3, 300, 400), max_soft=0.0, reformulate=False,
                 loss_fc=nn.CrossEntropyLoss()):
